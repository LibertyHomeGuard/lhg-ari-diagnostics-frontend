...
function ProfileCard({
  ...
  const phone = s(d.technician_phone) || claim.callerNumber || "";
  const email = s(d.technician_email);

  const displayName = company || claim.ticketNo;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-teal-700 text-sm font-bold text-white">
          {initial}
        </div>
        <div className="min-w-0 flex-1">
          ...
          {email ? (
            ...
            <p className="truncate text-xs text-neutral-500">
              {company ? claim.ticketNo : claim.propertyAddress}
            </p>
          )}
        </div>
      </div>
...
export default function ActiveClaimsList() {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      // Auto-select the first ticket if nothing is selected yet
      if (newRows.length > 0) {
        setSelectedTicketNo((prev) => prev ?? newRows[0].ticketNo);
      }
    } catch (e) {
  ...
  const handleSelectTicket = useCallback(
    (ticketNo: string) => {
      if (selectedTicketNo === ticketNo) return;
  ...
  useEffect(() => {
    ...
    // Both functions guard against duplicate calls internally; we only
    // want this effect to re-run when the selected ticket changes.
  ...
  const selectedDiagRecord = selectedTicketNo
    ? diagRecordByTicket[selectedTicketNo]
    : undefined;
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {filtered.map((claim) => (
            <ProfileCard
              key={claim.ticketNo}
              claim={claim}
              diagRecord={diagRecordByTicket[claim.ticketNo]}
              isSelected={selectedTicketNo === claim.ticketNo}
              onClick={() => handleSelectTicket(claim.ticketNo)}
            />
          ))}
